...
class ApiDumpGeneratorOptions(gen.GeneratorOptions):
    ...
    def __init__(self,
                 ...
                 alignFuncParam = 0,
                 genDirectory = None):
        gen.GeneratorOptions.__init__(self, filename, apiname, profile,
...
class ApiDumpOutputGenerator(gen.OutputGenerator):
    ...
    def beginFile(self, genOpts):
        ...
        for node in self.registry.find('extensions').findall('extension'):
    ...
    def endFile(self):
        ...
        self.sysTypes = set()
        for node in self.registry.find('types').findall('type'):
            if node.get('category') == None and node.get('requires') in self.includes and node.get('requires') != 'vk_platform':# and node.get('name') not in self.sysTypes:
                            ...
                            if struct.name == structName:
                                for member in struct.members:
                                    if node.get('name') == member.baseType or node.get('name') + '*' == member.baseType:
                                        #self.sysTypes[node.get('name')] = self.extTypes[structName].guard
                            ...
                            if func.name == funcName:
                                for param in func.parameters:
                                    if node.get('name') == param.baseType or node.get('name') + '*' == param.baseType:
                                        #self.sysTypes[node.get('name')] = self.extFuncs[funcName].guard
                                        sysType = VulkanSystemType(node.get('name'), self.extFuncs[funcName])
                                        if sysType not in self.sysTypes:
                                            self.sysTypes.add(sysType)
        #import pprint
        #pp = pprint.PrettyPrinter(indent=4)
        #pp.pprint(self.sysTypes)
