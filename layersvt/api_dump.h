...
public:
    ...
    inline bool shouldFlush() const
    {
        return should_flush;
    }
    
    inline bool showAddress() const
    {
        return show_address;
    }
    
    inline bool showParams() const
    {
        return show_params;
    }
...
public:
    ...
    inline loader_platform_thread_mutex* outputMutex()
    {
        return &output_mutex;
    }
    ...
    static inline ApiDumpInstance& current()
    {
...
template <typename T> inline void dump_text_array(const T* array, size_t len,
    const ApiDumpSettings& settings, const char* type_string, const char* child_type, const char* name, int indents,
    std::ostream& (*dump)(const T, const ApiDumpSettings&, int))
{
    ...
    {
        std::string indexName = std::string(name) + '[' + std::to_string(i) + ']';
...
template <typename T> inline void dump_text_pointer(const T* pointer, const ApiDumpSettings& settings,
...
template <typename T> inline void dump_text_value(const T object, const ApiDumpSettings& settings,
    const char* type_string, const char* name, int indents,
    std::ostream& (*dump)(const T, const ApiDumpSettings&, int))
{
    settings.formatNameType(settings.stream(), indents, name, type_string);
